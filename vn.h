...
void vn_label(int pos_x, int pos_y, int width, int height, char *fg_color, char *bg_color, char *text_style, char *str)
{
    ...
    else { vn_cprint(str, fg_color, bg_color, text_style); }
}

void vn_warn(int pos_x, int pos_y, int width, int height, char *fg_color, char *bg_color, char *frame_fg_color, char *frame_bg_color, char frame_vertical_symbol, char frame_horizontal_symbol, char *text_style, char *str, char *title)
{
    vn_bg(pos_x, pos_y, width, height, bg_color);
    vn_frame(pos_x, pos_y, width, height, frame_fg_color, frame_bg_color, frame_vertical_symbol, frame_horizontal_symbol);

    vn_gotoxy(pos_x+((width/2)-strlen(str)/2), pos_y);
    vn_cprint(title, fg_color, frame_bg_color, text_bold);

    vn_gotoxy(pos_x+((width/2)-strlen(str)/2), pos_y+((height-1)/2));
    vn_cprint(str, fg_color, bg_color, text_style);
}
#endif
